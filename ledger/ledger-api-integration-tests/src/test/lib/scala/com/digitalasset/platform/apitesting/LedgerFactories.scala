  ...
  def createRemoteSingleApiProxyResource(config: PlatformApplications.Config)(
    ...
    config.remoteApiEndpoint match {
      case Some(RemoteApiEndpointMode.Single(endpoint)) =>
        ...
        SingleRemoteServerResource.fromHostAndPort(endpoint.host, endpoint.port, endpoint.tlsConfig, config.ledgerId, packageIds, esf)
      case _ =>
        throw new AssertionError("config.remoteApiEndpoint has to be set and has to be RemoteApiEndpointMode.Single")
    }
  }
  ...
  def createRemoteMultiApiProxyResource(config: PlatformApplications.Config)(
    ...
    config.remoteApiEndpoint match {
      case Some(RemoteApiEndpointMode.MultiFromConfig(defaultParty, mappingConfig)) =>
        ...
        MultiRemoteServerResource.fromConfig(mappingConfig, defaultParty, config.ledgerId, packageIds, esf)
      case _ =>
        throw new AssertionError("config.remoteApiEndpoint has to be set and has to be RemoteApiEndpointMode.Multi")
    }
  }
