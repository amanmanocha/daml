...
object Cli {
  ...
  private val argParser = new scopt.OptionParser[Config]("ledger-api-test-tool") {
    ...
    opt[Int]('p', "target-port")
      ...
      .text("Server port of the Ledger API endpoint to test, if no mapping provided. Defaults to 6865.")

    opt[String]('h', "host")
      .action((x, c) => c.copy(host = x))
      .text("Server host of the Ledger API endpoint to test, if no mapping provided. Defaults to localhost.")
